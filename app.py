...
def load_index():
    ...
    if os.path.exists(INDEX_PATH):
        try:
            ...
            if index.d != dim:
                logger.warning(f"Index dimension {index.d} != model dimension {dim}. Creating new index.")
...
def process_pdf(file, company: str = "unknown") -> Dict[str, int]:
    try:
        ...
        if len(text.strip()) < 100:
            return {"added": 0, "duplicates": 0, "error": "PDF appears to be empty or unreadable."}
        
        chunks = create_smart_chunks(text)
        if not chunks:
            return {"added": 0, "duplicates": 0, "error": "No valid text chunks extracted from PDF."}
...
if st.session_state.history:
    ...
    st.sidebar.download_button("💾 Download Chat History", data=chat_export,
        file_name=f"chat_history_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt", mime="text/plain")
...
if query and len(metadata) > 0:
    with st.spinner("🔍 Searching and generating answer..."):
        ...
        if not chunks:
            ...
            prompt = f"""You are a financial analyst assistant. Use context from financial docs to answer clearly.
...
if st.session_state.history:
    ...
    with st.expander("📄 Sources Used", expanded=False):
        for i, c in enumerate(latest_chunks, 1):
            relevance = "High" if c.get('score', 1) < 0.5 else "Medium" if c.get('score', 1) < 1.0 else "Low"
            st.markdown(f"**Source {i}** - *{c.get('company', 'N/A')}* \n\n"
                        f"**Relevance:** {relevance} (Score: {c.get('score', 0):.3f})\n\n"
                        f"**Content:** {c['content'][:400]}{'...' if len(c['content']) > 400 else ''}")
            if i < len(latest_chunks): st.markdown("---")
...
st.markdown("---")
st.markdown(f"<div style='text-align: center; color: gray;'><small>"
            f"Financial RAG Bot v2.0 | FAISS + Sentence Transformers + Gemini<br/>"
            f"Model: {EMBEDDING_MODEL} | Chunks: {len(metadata)} | Companies: {len(companies)}"
