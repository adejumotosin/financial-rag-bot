...
# Gemini setup
GEMINI_MODEL_PRIMARY = "models/gemini-1.5-flash"
GEMINI_MODEL_FALLBACK = "models/gemini-1.5-pro"
...
def generate_with_gemini(prompt: str, max_tokens: int = 500) -> str:
    """Generate text with Gemini, Flash as default and Pro as fallback."""
    try:
        ...
        if "429" in err_msg or "quota" in err_msg.lower():
            try:
                ...
                return f"❌ Gemini fallback (Pro) also failed: {e2}"
        else:
...
st.markdown("""
Upload PDF financial reports and ask questions. The bot retrieves chunks and answers with Gemini.
""")
...
<small>Financial RAG Bot | Powered by FAISS, Sentence Transformers & Gemini</small>
</div>
